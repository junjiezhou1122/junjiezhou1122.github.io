...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place is not current_place.is_hive and current_place.entrance is not None:
            if(
                len(current_place.bees) > 0
                and dist >= self.lower_bound
                and dist <= self.upper_bound
            ):
...
# END Problem 12
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        if self.place == Water:
            return super().reduce_health(0)
        else:
            if amount >= self.health:
                ants_lose()
            return Insect.reduce_health(amount)
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = 2 - (distance * 0.25 + self.insects_shot * 0.0625)
        return max(0, damage)
        # END Problem Optional 2
